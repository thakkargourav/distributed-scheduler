...
public class GreedySchedulerStrategy extends AbstractSchedulerStrategy {
   ...
   */
  @Override
  public List<Map<ClusterMember, WorkloadActionsInstruction>> schedule(Set<? extends Workload> registeredWorkloads,
    ...
    Set<Workload> existingWorkloads = reports.values()
        .stream()
    ...
    Set<Workload> newWorkloads = registeredWorkloads.stream()
        .filter(workload -> !existingWorkloads.contains(workload))
        .collect(Collectors.toSet());

    // Determine the set of orphaned workloads.
    Set<Workload> orphanedWorkloads = existingWorkloads.stream()
        .filter(workload -> !registeredWorkloads.contains(workload))
        .collect(Collectors.toSet());
   ...
   */
  private Map.Entry<? extends ClusterMember, WorkloadReport> findLeastBusyMember(
   ...
   */
  private Map.Entry<? extends ClusterMember, WorkloadReport> findMostBusyMember(
      SchedulerStrategyContext context) {
    return context.getMapping()
        .entrySet()
        .stream()
